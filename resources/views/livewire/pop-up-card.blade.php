    ...
    <form action="{{ route('filter.products') }}" method="POST"
        ...
        <div class="mb-8 text-center">
            <h2 class="text-3xl font-bold text-gray-800 mb-2">
                {{ session('lang') == 'en' ? 'Price Filter' : 'فلترة أسعار  ' }}
            </h2>
            <p class="text-lg font-medium text-gray-600 mb-4">
                {{ session('lang') == 'en' ? 'Find within your budget' : 'ابحث ضمن ميزانيتك' }}
            </p>
        ...
        <div class="flex justify-between items-center gap-6 mb-8">
            <div class="flex-1">
                <label for="min_price" class="block mb-2 text-sm font-semibold text-gray-700">
                    {{ session('lang') == 'en' ? 'Min Price' : 'الحد الأدنى لسعر ' }}
                </label>
                <div class="relative">
                    <input type="text"
                        ...
                        name="min_price" id="min_price" placeholder="{{ session('lang') == 'en' ? '$' : '$' }}">
                </div>
            </div>

            <div class="flex-1">
                <label for="max_price" class="block mb-2 text-sm font-semibold text-gray-700">
                    {{ session('lang') == 'en' ? 'Max  Price' : 'الحد الأقصى لسعر ' }}
                </label>
                <div class="relative">
                    <input type="text"
                        class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-gray-500 focus:border-transparent placeholder-gray-400 transition-all"
                        name="max_price" id="max_price" placeholder="{{ session('lang') == 'en' ? '$' : '$' }}">
                </div>
            </div>
        </div>
