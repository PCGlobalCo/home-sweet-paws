<x-app-layout>
    ...
    {{-- End Breadcrumb  --}}
    <form class="pt-4" action="{{ route('service.update',$record->id) }}" enctype="multipart/form-data" method="POST">
        @csrf
        @method('PUT')
        <div class="grid gap-6 mb-6 md:grid-cols-1">
            <div class="grid grid-cols-1 gap-4 border-2 p-4 md:grid-cols-2">
                ...
                <div class="grid grid-cols-2 gap-2">
                    ...
                    <div>
                        <label for="name"
                            class="block mb-2 text-sm font-medium text-gray-900 d:text-white">{{ session('lang') == 'en' ? 'name' : 'الاسم الاجنبي' }}</label>
                    ...
                    <div class="">
                        ...
                        <input type="number" id="price" value="{{ $record->price }}" name="price" step=".01"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 d:bg-gray-700 d:border-gray-600 d:placeholder-gray-400 d:text-white d:focus:ring-blue-500 d:focus:border-blue-500"
                ...
                <div class="grid grid-cols-1 gap-4 border-2 p-4 md:grid-cols-2 md:col-start-1 md:col-end-3">
                    <div class="w-full mt-2">
                        <div class="flex justify-between items-center">
                            <label for="image_url"
                                class="block mb-2 text-sm font-medium text-gray-900 d:text-white">{{ session('lang') == 'en' ? 'image' : 'الصورة الرئيسية' }}</label>
                        </div>
                        <div class="mt-4">
            ...
            <div class="grid grid-cols-1 gap-4 border-2 p-4 md:grid-cols-2">
                <div class="">
                    <label for="description" class="block mb-2 text-sm font-medium text-gray-900 d:text-white">
                        {{ session('lang') == 'en' ? 'description' : 'الوصف بالاجنبي' }}</label>
                    <section>
                        <div
                            ...
                            <div class="px-4 py-2 bg-white rounded-b-lg d:bg-gray-800">
                                ...
                                <textarea id="description" rows="8" name="description"
                                    ...
                                    placeholder="">{{ $record->description }}</textarea>
                                <script>
